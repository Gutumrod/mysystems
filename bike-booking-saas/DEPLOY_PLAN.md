...
├── shop-frontend/          ← ระบบจองคิวลูกค้า (Next.js 15, port 3000)
├── shop-admin/             ← แดชบอร์ดร้าน (Next.js 15, port 3001)
...
|-----|---------------|--------|--------|
| `shop-frontend` | `craftbikelab-booking` | `booking.craftbikelab.com` | ลูกค้าจองคิว |
| `shop-admin` | `craftbikelab-booking-admin` | `booking-admin.craftbikelab.com` | เจ้าของร้าน |
...
```
ลูกค้า → booking.craftbikelab.com (shop-frontend)
               ↓
         Supabase (gsbbkdppaegrttcqmjuq)
         schema: bike_booking
         table: public_booking_slots (view)
               ↑
เจ้าของร้าน → booking-admin.craftbikelab.com (shop-admin)
               ↓ auth
...
> **สถานะปัจจุบัน:** ระบบยังใช้ `NEXT_PUBLIC_SHOP_ID` แบบ hardcode ต่อ deployment — wildcard subdomain routing ยังไม่ได้ implement ใน middleware ฝั่ง shop-frontend ต้องทำใน Phase ถัดไป

---

## 3. Vercel Project Setup

### ขั้นตอน: สร้าง Vercel Project สำหรับ shop-frontend
...
3. กำหนดค่า:
   ...
   - **Root Directory:** `shop-frontend` ← **สำคัญมาก**
   - **Build Command:** `next build` (default)
   - **Output Directory:** `.next` (default)
4. กด **Deploy**

### ขั้นตอน: สร้าง Vercel Project สำหรับ shop-admin
...
3. กำหนดค่า:
   ...
   - **Root Directory:** `shop-admin` ← **สำคัญมาก**
...
### shop-frontend — Production Environment Variables
...
### shop-admin — Production Environment Variables
...
### shop-frontend: ไม่มี middleware

`shop-frontend` **ไม่มีไฟล์ `middleware.ts`** เลย ทุก route เปิดให้ลูกค้าเข้าได้โดยไม่ต้อง login — ถูกต้องแล้ว ไม่ต้องแก้ไข

### shop-admin: middleware.ts วิเคราะห์

ไฟล์: `shop-admin/middleware.ts`
...
ปัจจุบัน `shop-frontend` ใช้ `NEXT_PUBLIC_SHOP_ID` แบบ hardcode ต่อ deployment  
เมื่อต้องการให้หลายร้านใช้ระบบเดียวกัน ต้องเพิ่ม middleware ใน `shop-frontend`:

```ts
// shop-frontend/middleware.ts (ยังไม่มีในตอนนี้)
...
- **`shop-frontend/.gitignore`** — ไม่มีไฟล์ → **สร้างใหม่แล้ว** ✅
- **`shop-admin/.gitignore`** — ไม่มีไฟล์ → **สร้างใหม่แล้ว** ✅
...
- **`shop-frontend/.env.production.example`** — **สร้างใหม่แล้ว** ✅ (3 ตัวแปร)
- **`shop-admin/.env.production.example`** — **สร้างใหม่แล้ว** ✅ (4 ตัวแปร รวม `SUPABASE_SERVICE_ROLE_KEY`)

ไฟล์เหล่านี้ commit ได้ เพราะไม่มีค่าจริง ใช้เป็น reference ตอนตั้งค่า Vercel

**3. ตรวจสอบ next.config.ts ทั้ง 2 apps**

| ประเด็น | shop-frontend | shop-admin |
|--------|--------------|------------|
...
| ไฟล์ | การแก้ไข |
|------|---------|
| `shop-frontend/.gitignore` | สร้างใหม่ — เพิ่ม node_modules, .next, .env*.local |
| `shop-admin/.gitignore` | สร้างใหม่ — เพิ่ม node_modules, .next, .env*.local |
| `shop-frontend/.env.production.example` | สร้างใหม่ — ตัวอย่างตัวแปร production พร้อมคำอธิบายไทย |
| `shop-admin/.env.production.example` | สร้างใหม่ — ตัวอย่างตัวแปร production พร้อมคำอธิบายไทย |
...
```bash
git rm --cached shop-frontend/.env.local
git rm --cached shop-admin/.env.local
```

**[2] Commit ไฟล์ใหม่ที่สร้างวันนี้**

```bash
git add shop-frontend/.gitignore shop-admin/.gitignore
git add shop-frontend/.env.production.example shop-admin/.env.production.example
...
- **service_role (secret)** → ใช้เป็น `SUPABASE_SERVICE_ROLE_KEY` (เฉพาะ shop-admin เท่านั้น ห้ามใส่ใน shop-frontend)
